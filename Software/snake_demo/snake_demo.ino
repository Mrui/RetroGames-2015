// Snake prototype with timer and all the buttons working.
/*
 * What is working?
 * - Movement of the snake and collision checks. nothing else.
 * TODO:
 * - Spawning and collection of "fruits" or w/e, simple random() command on fruit pickup.
 * - Making the tail vanish and the snake grow
...
void loop()
{
  ...
  while (1)
  {
    ...
    else if (digitalRead(butUp) == LOW && butUpLast == HIGH)
    {
      pushedButton = 0;
      butUpLast == LOW;
    }
    ...
    if (counter == 250)
    {
      ...
      switch ( direction ) {
        case 0: // up
          ...
          checkCollision(xPos+1, yPos);
        break;
        case 1: // right
          if (xPos < 81)
              xPos = xPos+1;
          checkCollision(xPos+1, yPos);
          checkCollision(xPos+1, yPos+1);
        break;
        case 2: // down
          if (yPos < 45)
            yPos = yPos+1;
      ...
      setPixel(xPos+1, yPos, BLACK);
      setPixel(xPos+1, yPos+1, BLACK);
      setPixel(xPos, yPos+1, BLACK);
    }
    if (pushedButton == 4) {
      direction = 1;
    }
    if (pushedButton == 0 || valueJoyY < joyMiddle-100) {
        direction = 0;
    }
    if (pushedButton == 1 || valueJoyX > joyMiddle+100) {
        direction = 1;
    }
    if (pushedButton == 2 || valueJoyY > joyMiddle+100) {
        direction = 2;
    }
    if (pushedButton == 3 || valueJoyX < joyMiddle-100) {
        direction = 3;
    }
  }
}
...
void lcdBegin(void)
{
  ...
  Timer1.initialize(30000);                // trying to set a timer of length 10000 microseconds or 0.01 sec tick
  Timer1.attachInterrupt(timerISR,30000);  // trying to attach the service routine with the same - to execute on every tick
